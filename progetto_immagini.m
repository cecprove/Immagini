...
for i=1:size(lista_tipologie,2)
    %prendo tutti i nomi dalla struct e li confronto con quelli presemte in
    %lista_stringhe
    count = 0;
...
thumbnail_16x16= struct;
for i=1:size(ImagesPath,1)
    thumbnail_16x16(i).thumbnail_16x16=imresize(images(i).images,[16 16]);
end
% per prova: imshowpair(images(7).images,thumbnail_16x16(7).thumbnail_16x16,'montage')
...
% mettiamo in thumnail_rif sia l'immagine sia l'espressione corrispondente
% sia il soggetto
stringa_sub = '01'; %char
...
for k=1:size(ImagesPath,1)
    ...
    for i=1:size(lista_stringhe,1)
        ...
        if strcmp(extractAfter(ImagesPath(k).name,"."),lista_stringhe(i))
            ...
            thumbnail_16x16(k).tipologia = extractAfter(ImagesPath(k).name,".");
...
for i=1:size(thumbnail_16x16,2)
    switch thumbnail_16x16(i).tipologia
        case 'glasses'
           thumbnail_16x16(i).etichetta = 1; 
        case 'happy'
            thumbnail_16x16(i).etichetta = 2;
        case 'leftlight'
            thumbnail_16x16(i).etichetta = 3;
        case 'noglasses'
            thumbnail_16x16(i).etichetta = 4;
        case 'normal'
            thumbnail_16x16(i).etichetta = 5;
        case 'rightlight'
            thumbnail_16x16(i).etichetta = 6;
        case 'sad'
            thumbnail_16x16(i).etichetta = 7;
        case 'sleepy'
            thumbnail_16x16(i).etichetta = 8;
        case 'surprised'
            thumbnail_16x16(i).etichetta = 9;
        case 'wink'
            thumbnail_16x16(i).etichetta = 10;
        case 'centerlight'
            thumbnail_16x16(i).etichetta = 11;      
    end
end

%ora creiamo la thumbnail
for i=1:size(thumbnail_rif,2)
    thumbnail_rif(i).thumbnail_rif_space=imresize(thumbnail_rif(i).thumbnail_rif_space,[16 16]);
  %  figure
...
%creo dominio di riferimento (DCT2) %%controlla bene, possiamo mettere
%spettro ma non bisogna trasformare in double prima
for i=1:size(thumbnail_rif,2)
...
for i=1:size(thumbnail_16x16,2)
     feature(i).media =  mean(mean(thumbnail_16x16(i).thumbnail_16x16));
     feature(i).entropia = entropy(thumbnail_16x16(i).thumbnail_16x16);
     feature(i).mediaparti = diff_media_totImm(i);
     feature(i).simmetria = skewness(skewness(double(thumbnail_16x16(i).thumbnail_16x16)));
end
 
for i=1:size(thumbnail_rif,2)
  trovato = 0;
  for k=1:size(thumbnail_16x16,2)
      if thumbnail_rif(i).thumbnail_rif_space == thumbnail_16x16(k).thumbnail_16x16 
          thumbnail_16x16_senza_rif(k).thumbnail_16x16 = zeros(16);
          feature(k).media = 0;
          ...
          if trovato == 1
          thumbnail_16x16_senza_rif(k).thumbnail_16x16 = thumbnail_16x16(k).thumbnail_16x16;
          end
      end
  end
end

for i=1:size(thumbnail_16x16_senza_rif,2)
    if mean(mean(double(thumbnail_16x16_senza_rif(i).thumbnail_16x16) == zeros(16))) ~= 1 
        thumbnail_16x16_senza_rif(i).tipologie = thumbnail_16x16(i).tipologia;
        thumbnail_16x16_senza_rif(i).etichetta = thumbnail_16x16(i).etichetta;
    end
end
...
%plot delle immagini di riferimento
 ...
 % for i=1:size(feature_rif,2)
  %    scatter(feature_rif(i).media,feature_rif(i).entropia)
   %   text(feature_rif(i).media,feature_rif(i).entropia,string(feature_rif(i).etichetta),'VerticalAlignment','bottom')
      
    %  hold on
  %end

 for k= 1:size(thumbnail_16x16_senza_rif,2)
     if rank(double(thumbnail_16x16_senza_rif(k).thumbnail_16x16)) ~= 0
         for i= 1: size(thumbnail_rif,2)
 ...
 for k= 1:size(thumbnail_16x16_senza_rif,2)
     if sum(distanze(:,k)) ~= 0
        ...
        verita(k).etichetta_vera = thumbnail_16x16_senza_rif(k).etichetta;
...
for i=1:size(thumbnail_16x16,2)
   [~, threshold] = edge(thumbnail_16x16(i).thumbnail_16x16, 'sobel');
   fudgeFactor = .2;
   elab_immagini(i).imm_contorni = edge(thumbnail_16x16(i).thumbnail_16x16,'sobel', threshold * fudgeFactor);
...
background= struct;
for i=1:size(thumbnail_16x16,2)
   elab_immagini(i).binary = imbinarize(thumbnail_16x16(i).thumbnail_16x16)
%prendo le due fasce laterali dell'immagine   
