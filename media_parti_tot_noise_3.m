function diff_media_totImm_noise_3 = media_parti_tot_noise_3(thumbnail,lunghezza_tot)
...
for i=1:lunghezza_tot
   [~, threshold] = edge(thumbnail.thumbnail_16x16_senza_rif(i).noise_16x16_3, 'sobel');
   fudgeFactor = .2;
   elab_immagini(i).imm_contorni = edge(thumbnail.thumbnail_16x16_senza_rif(i).noise_16x16_3,'sobel', threshold * fudgeFactor);
...
background= struct;
for i=1:lunghezza_tot
   elab_immagini(i).binary = imbinarize(thumbnail.thumbnail_16x16_senza_rif(i).noise_16x16_3);
...
diff_media_totImm_noise_3 = media_partesx - media_partedx;
